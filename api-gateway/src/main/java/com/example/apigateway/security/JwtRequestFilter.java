...
public class JwtRequestFilter implements Filter{
    ...
    public JwtRequestFilter() {
        System.out.println("✅ JwtRequestFilter initialized v1!");
    }

    @PostConstruct
    public void init() {
        System.out.println("✅ JwtRequestFilter initialized v2!");
    }
...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    // This method is automatically invoked for every incoming request
    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (jwtToken != null && jwtToken.startsWith("Bearer ")) {
            ...
            if (username != null) {
                // Retrieve the secret key from Redis for this user
                String secretKeyBase64 = jwtRedisService.retrieveSecretKey(username);
                if (secretKeyBase64 != null) {
                    String storedToken = new String(Base64.getDecoder().decode(secretKeyBase64), StandardCharsets.UTF_8);
                    if (storedToken.equals(jwtToken)) {
                        // Token is valid, set authentication in the security context
                        SecurityContextHolder.getContext().setAuthentication(
                                new UsernamePasswordAuthenticationToken(username, null, null));
                        System.out.println("User is authenticated.");
                    } else {
